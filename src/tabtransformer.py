...
class TabTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        shared_categ_dim_divisor=8.0,  # in paper, they reserve dimension / 8 for category shared embedding
    ):
        super().__init__()
        assert all(
        ...
        shared_embed_dim = (
            0 if not use_shared_categ_embed else int(dim // shared_categ_dim_divisor)
        )
